...
export class Webflow {
    ...
    site({ siteId }, query = {}) {
        if (!siteId)
            return Promise.reject(buildRequiredArgError('siteId'));
    ...
    publishSite({ siteId, domains }) {
        if (!siteId)
            return Promise.reject(buildRequiredArgError('siteId'));
        if (!domains)
            return Promise.reject(buildRequiredArgError('domains'));
    ...
    domains({ siteId }) {
        if (!siteId)
            return Promise.reject(buildRequiredArgError('siteId'));
        return this.get(`/sites/${siteId}/domains`).then(domains => domains.map(domain => this.responseWrapper.domain(domain)));
    }
    // Collections
    collections({ siteId }, query = {}) {
        if (!siteId)
            return Promise.reject(buildRequiredArgError('siteId'));
        return this.get(`/sites/${siteId}/collections`, query).then(collections => collections.map(collection => this.responseWrapper.collection(collection)));
    }
    collection({ collectionId }, query = {}) {
        if (!collectionId)
            return Promise.reject(buildRequiredArgError('collectionId'));
        return this.get(`/collections/${collectionId}`, query).then(collection => this.responseWrapper.collection(collection));
    }
    // Items
    ...
    item({ collectionId, itemId }, query = {}) {
        if (!collectionId)
            return Promise.reject(buildRequiredArgError('collectionId'));
        if (!itemId)
            return Promise.reject(buildRequiredArgError('itemId'));
        return this.get(`/collections/${collectionId}/items/${itemId}`, query).then(res => this.responseWrapper.item(res.items[0], collectionId));
    }
    createItem({ collectionId, ...data }, query = {}) {
        if (!collectionId)
            return Promise.reject(buildRequiredArgError('collectionId'));
        return this.post(`/collections/${collectionId}/items`, data, query).then(item => this.responseWrapper.item(item, collectionId));
    }
    ...
    webhooks({ siteId }, query = {}) {
        if (!siteId)
            return Promise.reject(buildRequiredArgError('siteId'));
        return this.get(`/sites/${siteId}/webhooks`, query).then(webhooks => webhooks.map(webhook => this.responseWrapper.webhook(webhook, siteId)));
    }
    webhook({ siteId, webhookId }, query = {}) {
        if (!siteId)
            return Promise.reject(buildRequiredArgError('siteId'));
        if (!webhookId)
            return Promise.reject(buildRequiredArgError('webhookId'));
        return this.get(`/sites/${siteId}/webhooks/${webhookId}`, query).then(webhook => this.responseWrapper.webhook(webhook, siteId));
    }
    createWebhook({ siteId, ...data }, query = {}) {
        if (!siteId)
            return Promise.reject(buildRequiredArgError('siteId'));
        return this.post(`/sites/${siteId}/webhooks`, data, query).then(webhook => this.responseWrapper.webhook(webhook, siteId));
    }
